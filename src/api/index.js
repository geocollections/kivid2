...
function applyMode(mode, separator = "=", prefix = "", queryJoiner = "&") {
  ...
  if (mode === "in_estonia")
    returnVal = `${queryJoiner}${prefix}in_estonia${separator}1`;
  return returnVal;
}
...
export function fetchHierarchy(id, hierarchyString, clsId) {
  return fetch(
    `rock_tree/?rock_id!=${id}&rock_id__in=${hierarchyString}&rock_classification_id=${clsId}&format=json`
  );
}
export function fetchRockTree(clsId, parentId, mode) {
  return fetch(
    `rock_tree/?rock_classification_id=${clsId}&parent_id=${parentId}&rock__name__isnull=False&rock__name_en__isnull=False&format=json`
  );
}
export function fetchRockSiblings(clsId, id, mode) {
  return fetch(
    `rock_tree/?rock_classification_id=${clsId}&parent_id=${id}&rock__name__isnull=False&rock__name_en__isnull=False&format=json`
  );
}
export function cntSpecimenCollection(id) {
  return fetch(
    `solr/specimen/?fq=hierarchy_string_rock:${id}&rows=1&format=json`
  );
}
export function fetchSpecimenCollection(id, mode, searchParameters) {
  ...
  let isFq =
    searchParameters.specimens.onlyImgs === true ||
    searchParameters.specimens.git === true ||
    searchParameters.specimens.tug === true ||
    searchParameters.specimens.elm === true;
  let fq = "";
  if (isFq === true) {
    fq += "&fq=";
    fq +=
      searchParameters.specimens.onlyImgs === true
        ? "image_preview_url:[%27%27 TO *]"
        : "";
    if (
      searchParameters.specimens.git === true ||
      searchParameters.specimens.tug === true ||
  ...
  return fetch(
    `solr/specimen/?fq=hierarchy_string_rock:${id}${fq}&rows=${searchParameters.specimens.paginateBy
    }&start=${start}&sort=${orderBy}&format=json`
  );
}
export function fetchSearch(name, mode) {
  ...
  return fetch(
    `rock/?sql=simple_rock_search&keyword=${name}${applyMode(mode)}&format=json`
  );
}
export function fetchLastChangedRocks(mode) {
  //return fetch(`rock/?multi_search=value:${name};fields:name,name_en;lookuptype:icontains&fields=id,name,name_en${applyMode(mode)}&format=json`)
  return fetch(
    `rock/?fields=id,name,name_en,name_ru,date_changed&paginate_by=5&page=1&order_by=-date_changed&format=json`
  );
}
export function fetchPhotoGallery(parentString, mode) {
  return fetch(
    `rock/?sql=rock_photo_gallery&keyword=${parentString}&page=1&paginate_by=25&format=json`
  );
}
...
export function fetchSearchByMineral(q, numberOfParams, mode) {
  ...
  return fetch(
    `rock/?sql=rock_mineral_search&keyword=${q}&paginate_by=${numberOfParams}${applyMode(
      mode
    )}&format=json`
  );
}
export function fetchSearchByChemicalElement(q, mode) {
  ...
  return fetch(
    `rock/?${q}${applyMode(
      ...
      "=",
      ""
    )}&fields=id,name,name_en,name_ru&format=json`
  );
}
export function fetchMineralList() {
  return fetch(
    `rock_mineral/?fields=mineral__id,mineral__name,mineral__name_en,mineral__name_ru&distinct=true&format=json`
  );
}
export function fetchChemicalElementList() {
  return fetch(
    `rock_element/?fields=element,element__element,element__name,element__name_en,element__name_ru&distinct=true&format=json`
  );
}
export function fetchSearchByPropertyType(properties, numberOfParams) {
  ...
  return fetch(
    `rock/?sql=rock_property_search&keyword=${properties}&paginate_by=${numberOfParams}&format=json`
  );
}
export function fetchRockElement(id) {
  return fetch(
    `rock_element/?rock=${id}&fields=element__element,content&order_by=-content&format=json`
  );
}
